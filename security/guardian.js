...
module.exports = options => {
  return [
    ...
    (req, res, next) => {
      if (
        !req.payload ||
        !(options.mailToken ? req.payload.uid : options.activationToken ? req.payload.acuid : req.payload.id)
      ) {
        ...
        usuarioModel
          .findOne({
            where: {
              id: options.mailToken ? req.payload.uid : options.activationToken ? req.payload.acuid : req.payload.id
            }
          })
          .then(
            usuario => {
